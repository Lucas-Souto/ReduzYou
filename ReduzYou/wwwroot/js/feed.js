...
function addPost(post)
{
    main.insertAdjacentHTML('beforeend', `
        <a class="post" href="/${post.author}/${post.link}">
            <img class="post-img" src=${post.cover.length > 0 ? post.cover : "/img/no_image.svg"} />
            <div class="post-bottom">
                <h3 class="post-title">${post.title}</h3>
                <div class="post-stars">
                    <h4 class="star-number">${(post.starCount == 0 ? 0 : (post.totalValue / post.starCount) / 10).toString().substring(0, 3) }</h4>
